...
launch_args = [
    ...
    DeclareLaunchArgument(
        name="config_path",
        default_value="",
        description="path to estimator_config_no_loop.yaml. If not given, determined based on provided 'config' above",
    ),
    DeclareLaunchArgument(
        name="verbosity",
        default_value="INFO",
...
def launch_setup(context):
    ...
    if not config_path:
        ...
        if config in available_configs:
            config_path = os.path.join(
                            ...
                            "config",config,"estimator_config_no_loop.yaml"
                        )
        else:
            return [
                LogInfo(
    ...
    # OpenVINS node without loop closure
    node1 = Node(
        package="ov_msckf",
        ...
        remappings=[
            # Camera topics - map from OpenVINS internal topics to bag topics
    ...
    node2 = Node(
        ...
        arguments=[
            "-d",
            os.path.join(get_package_share_directory("ov_msckf"), "launch", "display.rviz"),
            "--ros-args",
            "--log-level",
            "warn",
        ],
