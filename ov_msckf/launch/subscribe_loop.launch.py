...
launch_args = [
    ...
    DeclareLaunchArgument(
        name="config_path",
        default_value="",
        description="path to estimator_config_loop.yaml. If not given, determined based on provided 'config' above",
    ),
    DeclareLaunchArgument(
        name="verbosity",
        default_value="INFO",
    ...
    # Loop closure specific arguments
    DeclareLaunchArgument(
        name="loop_enabled",
        default_value="true",
    ...
    DeclareLaunchArgument(
        name="loop_vocab_path",
        default_value="",
        description="path to BRIEF vocabulary file (empty = use default vocabulary)",
    ),
]
...
def launch_setup(context):
    ...
    if not config_path:
        ...
        if config in available_configs:
            config_path = os.path.join(
                            ...
                            "config",config,"estimator_config_loop.yaml"
                        )
        else:
            return [
                LogInfo(
    ...
    # OpenVINS node with loop closure parameters
    node1 = Node(
        package="ov_msckf",
    ...
    # RViz node with loop closure display
    node2 = Node(
        package="rviz2",
        executable="rviz2",
        ...
        arguments=[
            "-d",
            os.path.join(get_package_share_directory("ov_msckf"), "launch", "display_loop.rviz"),
            "--ros-args",
            "--log-level",
            "warn",
        ],
