...
bool State_JPLQuatLocal::Minus(const double *y, const double *x, double *y_minus_x) const {
  // Compute quaternion difference: q_diff = q_y * q_x^{-1}
  Eigen::Map<const Eigen::Vector4d> q_y(y);
  Eigen::Map<const Eigen::Vector4d> q_x(x);

  // Compute inverse of x quaternion (negate xyz components for JPL)
  Eigen::Vector4d q_x_inv;
  q_x_inv << -q_x(0), -q_x(1), -q_x(2), q_x(3);
  q_x_inv = ov_core::quatnorm(q_x_inv);

  // Compute difference quaternion
  Eigen::Vector4d q_diff = ov_core::quat_multiply(q_y, q_x_inv);

  // Convert to axis-angle representation (tangent space)
  Eigen::Map<Eigen::Vector3d> delta(y_minus_x);
  double theta = 2.0 * std::atan2(q_diff.head<3>().norm(), std::abs(q_diff(3)));
  if (theta < 1e-8) {
...
bool State_JPLQuatLocal::MinusJacobian(const double *x, double *jacobian) const {
  // For quaternion manifold, the minus jacobian is typically identity
  // This is a simplified implementation - more sophisticated versions exist
